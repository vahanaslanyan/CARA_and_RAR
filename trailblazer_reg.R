...
#LSM change in high group active group was -18.29 (95% CI: (-20.61 to -15.97)) (ITT N=165) SD=15.2)
#LSM change in high group active group was -19.23 (95% CI: (-21.43 to -17.03)) (ITT N=208) SD=16.2)
...
model_regression <- cmdstan_model("./regression_trailblazer.stan")
simulate_data<- function(N=1736, allocation_proportion=c(0.5,0.5)) {
  ...
  data_long$y1[data_long$tx=="Active" & data_long$tx==0]<-data_long$y1[data_long$tx=="Active" & data_long$tx==0]+
  ...
  data_long$y1[data_long$tx=="Active" & data_long$tau_load==1]<-data_long$y1[data_long$tx=="Active" & data_long$tau_load==1]+
...
interim_steps<-function(data_cara,data_rar,tuning_parameter=nrow(data_cara)/(1736*2)){
    ...
    # fit the models 
    fit_cara <- model_regression$sample(data = stan_data_cara,chains=5,parallel_chains = 10,refresh = 0)
    fit_rar<-model_regression$sample(data=stan_data_rar,chains=5,parallel_chains = 10,refresh = 0)
...
interim_data_generation<-function(N=40,K=c("Placebo", "Active"),data_cara,data_rar,
                                  interim_df,true_coefficients=true_coefficients1,
                                  tuning_parameter=nrow(data_cara)/(1736*2)){
  #generate new data
...
for(i in 1:5000){
  ...
  aux_effect_sizes$effect_size_cara_second_iteration_age<-get_effect_size(data_cara)[2]
  aux_effect_sizes$effect_size_rar_second_iteration_tau<-get_effect_size(data_rar)[1]
  aux_effect_sizes$effect_size_rar_second_iteration_age<-get_effect_size(data_rar)[2]
  ...
  aux_effect_sizes$effect_size_cara_third_iteration_age<-get_effect_size(data_cara)[2]
  aux_effect_sizes$effect_size_rar_third_iteration_tau<-get_effect_size(data_rar)[1]
  aux_effect_sizes$effect_size_rar_third_iteration_age<-get_effect_size(data_rar)[2]
  ...
  fit_cara <- model_regression$sample(data = cara_data_final,chains=5,parallel_chains = 10,refresh = 0)
  ...
  fit_rar <- model_regression$sample(data = rar_data_final,chains=5,parallel_chains = 10,refresh = 0)
...
write_csv(effect_sizes,paste0(OUTPUT_PATH,"effect_sizes_trailblazer_25_50.csv"))
write_csv(allocation_probs,paste0(OUTPUT_PATH,"allocation_probabilities_trailblazer_25_50.csv"))
write_csv(betas_cara,paste0(OUTPUT_PATH,"beta_cara_trailblazer_25_50.csv"))
write_csv(betas_rar,paste0(OUTPUT_PATH,"beta_rar_trailblazer_25_50.csv"))
